<template>
  <div class="navbar">
      <section v-on:click="home" class="navbar-logo">
        <img class="navbar-logo-img" src="data:image/png;base64," alt="logo">
        <h1>VueTube</h1>
      </section>

      <section class="navbar-search">
        <input type="text" placeholder="Search">
        <button><i class="fas fa-search"></i></button>
      </section>

      <section class="navbar-right">
        <button v-on:click="nav"><i class="fas fa-user"></i>SIGN IN</button>
      </section>
  </div>
</template>

<script>
export default {
  name: "Navbar",
  methods: {
    nav: function() {
      this.$router.push('/login')
    },
    home: function() {
      this.$router.push('/')
    }
  }
}
</script>

<!-- Add "scoped" attribute to limit CSS to this component only -->
<style scoped>
.navbar, .navbar-logo, .navbar-search, .navbar-right {
  align-items: center;
  display: flex;
}

.navbar {
  height: 2em;
  padding: 1em;
  max-width: 80%;
  min-width: 800px;
  margin: 0 auto;
  justify-content: space-between;
  background-color: white;
}

.navbar-logo {
  height: 100%;
  font-weight: bolder;
  font-size: 1em;
  flex: 1;
}

.navbar-logo:hover {
  cursor: pointer;
}

.navbar-logo-img {
  height: 55%;
  margin-right: 0.25em;
}

.navbar-search { 
  flex: 2;
}

.navbar-search input {
  padding: 0.25em 0.5em;
  width: 80%;
  border: 1px solid lightgray;
  border-top-left-radius: 2px;
  border-bottom-left-radius: 2px;
}

.navbar-search button {
  padding: 0.46em 2em 0.48em 2em;
  display: block;
  border: 1px solid lightgray;
  border-top-right-radius: 2px;
  border-bottom-right-radius: 2px;
  border-left: 0;
  font-size: 0.80em;
  color: darkgray;
  background-color: rgb(245, 245, 245);
}

.navbar-search button:hover {
  background-color: rgb(230, 230, 230);
}

.navbar-right {
  flex: 1;
}

.navbar-right button {
  border: 1px solid rgb(72, 72, 194);
  color: rgb(72, 72, 194);
  padding: 0.5em 0.75em;
  border-radius: 2px;
  margin-left: auto;
  font-size: 0.90em;
  font-weight: bold;
}

.navbar-right i {
  margin-right: 1em;
  background-color: rgb(72, 72, 194);
  color: white;
  border-radius: 50%;
  padding: 0.43em 0.46em;
}
</style>
